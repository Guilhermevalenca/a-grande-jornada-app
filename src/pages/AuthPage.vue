    ...
    <q-card-section class="flex justify-end">
      <q-btn icon="mdi-help" round color="secondary" size="sm">
        <q-tooltip :hide-delay="1000">
          Digite seu email para fazer login, caso não esteja logado você será
          direcionado para a tela de cadastro
        </q-tooltip>
      </q-btn>
    </q-card-section>
    <q-card-section>
      <q-tab-panels v-model="panel">
        <q-tab-panel name="start">
          <q-form @submit.prevent="submit">
            ...
            <div class="flex justify-end">
              <q-btn type="submit" color="primary" :loading="loading" push
                >Prosseguir</q-btn
              >
            </div>
          </q-form>
        ...
        <q-tab-panel name="login">
          <LoginComponent :email="email" @previousForm="panel = 'start'" />
        </q-tab-panel>
        <q-tab-panel name="create">
          <CreateComponent :email="email" @previousForm="panel = 'start'" />
        </q-tab-panel>
      </q-tab-panels>
    </q-card-section>
  </q-card>
...
export default defineComponent({
  ...
  components: {
    CreateComponent: defineAsyncComponent(
      () => import('components/auth/CreateComponent.vue')
    ),
