<template>
  ...
  <q-item-section class="flex items-end">
    <q-icon name="mdi-eye-outline" flat size="sm" />
  </q-item-section>
  <q-dialog
  ...
  >
    <q-card>
      ...
      <q-inner-loading :showing="loadingUpdateTitle" label="Carregando..." />
    </q-card>
  </q-dialog>
    ...
    <q-card class="q-pr-sm q-pl-sm q-pb-sm">
      <q-card-section>
        <div class="flex justify-end">
          <q-btn icon="close" flat v-close-popup :disable="loading" />
        </div>
        <div class="text-h6 text-center">Tem certeza?</div>
      </q-card-section>
      <q-card-section>
        <div>
          Ao deletar o formulário: "<strong>{{ form.title }}</strong
          >" todas as questões e suas alternativas serão perdidos.
        </div>
      </q-card-section>
      <q-card-actions class="flex justify-end">
        <q-btn v-close-popup :disable="loading">cancelar</q-btn>
        <q-btn
          color="negative"
          @click="deleteForm"
          :disable="loading"
...
export default defineComponent({
  ...
  components: {
    RenderQuestion: defineAsyncComponent(
      () => import('components/form/listForms/RenderQuestionFormComponent.vue')
    ),
  },
  ...
  methods: {
    ...
    async updateForm() {
      ...
      await this.$api
        .patch(`api/form/${this.form.id}`, this.editForm)
        .then((response) => {
          if (response.data) {
            this.$emit('allForms');
            this.$q.notify({
